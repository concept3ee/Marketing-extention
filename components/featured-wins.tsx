...
const FeaturedWins: React.FC = () => {
  const wins = [
    { name: 'Need to meet tight deadlines but don’t have enough hands on deck?', metric: '+120% MQLs', industry: 'Fintech', region: 'Nigeria', delay: 0.1 },
    { name: 'Looking for a design and development partner that scales with you?', metric: 'CAC reduced 35%', industry: 'Health', region: 'Kenya', delay: 0.2 },
    { name: 'Want to turn your MVP into a product that dominates the market?', desc: 'SaaS platform for remote engineering collaboration.', metric: '3x Pipeline', industry: 'SaaS', region: 'Global', delay: 0.3 },
  ...
  return (
    <section className="bg-bg-light pt-32">
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="mb-20">
          <p className="text-text-muted text-xs font-bold uppercase tracking-widest mb-6"> Building a Startup Is Hard. Finding the Right Partner Shouldn’t Be.</p>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 border-l border-t border-gray-100">
        {wins.map((win, idx) => (
          ...
          <WinCard key={win.name} {...win as any} />
        ))}
      </div>
    </section>
  );
